-- Forward-only schema v4 to v5. Apply once in a host-owned transaction
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            ERRCODE = '23514',
            MESSAGE = 'v5 cutover would strand terminal host-charge targets',
            HINT = 'Roll back and remain on v4. Run the v5 preflight and audit; a supported recovery or compatibility policy is required before retrying.';
    END IF;
END
$$;

-- Validate retained reversal tuples before adding classification columns.
...
-- that v4 observed an authoritative approval or a structured approval field.
...
-- Rebind ledger admission to the v5 evidence classification. Every path that
