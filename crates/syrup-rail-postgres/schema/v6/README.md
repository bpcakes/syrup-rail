...
An exact provider query that returns a matching transaction can replace an
unresolved attempt's observation with a newly classified one. Empty or malformed
query results do not prove absence and never downgrade retained evidence. If
...
upgrade on a representative copy, and drain billing traffic. Stop **all** billing
...
`V6_INSTALL_SQL` and `V5_TO_V6_UPGRADE_SQL` are available through the explicit
`schema-contract-test-support` feature. Once released, this guide and both SQL
