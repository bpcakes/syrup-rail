...
-> Result<(), Box<dyn Error>> {
    ...
    for statement in [CANDIDATE_SQL, LOCKED_CANDIDATE_SQL] {
        ...
        assert_supersession_index(root, &rendered);
...
#[tokio::test]
async fn card_metadata_candidate_plans_bound_work_to_one_dense_method_history()
-> Result<(), Box<dyn Error>> {
    let database = TestDatabase::start("meta_dense_plan").await?;
    sqlx::raw_sql(include_str!("query_plan_fixture.sql"))
        .execute(&database.pool)
        .await?;
    sqlx::raw_sql(include_str!("dense_history_fixture.sql"))
        .execute(&database.pool)
        .await?;
    let history: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM billing_payment_attempts WHERE payment_method_id = md5('method1')::uuid",
    ).fetch_one(&database.pool).await?;
    assert_eq!(history, 1025);
    for statement in [CANDIDATE_SQL, LOCKED_CANDIDATE_SQL] {
        let explain = format!("EXPLAIN (GENERIC_PLAN TRUE, FORMAT JSON) {statement}");
        let row = sqlx::raw_sql(&explain).fetch_one(&database.pool).await?;
        let plan: Value = row.try_get(0)?;
        let root = explain_plan_root(&plan)?;
        assert_supersession_index(root, &serde_json::to_string_pretty(root)?);
        for (transaction, expected_rows) in [("txn_1", 1), ("history_txn_1024", 0)] {
            let (scope, subscriber, attempt): (Uuid, Uuid, Uuid) = sqlx::query_as(
                "SELECT billing_scope_id, subscriber_id, id FROM billing_payment_attempts WHERE gateway_transaction_id = $1",
            ).bind(transaction).fetch_one(&database.pool).await?;
            let mut tx = database.pool.begin().await?;
            crate::enrollment_application::set_application_timeouts(&mut tx).await?;
            ...
            let root = explain_plan_root(&plan)?;
            assert_eq!(
                root["Actual Rows"].as_f64(),
                Some(f64::from(expected_rows)),
                "{plan}"
            );
            let newer = find_alias(root, "newer").expect("supersession lookup is present");
            let visited = newer["Actual Rows"].as_f64().unwrap()
                + newer["Rows Removed by Filter"].as_f64().unwrap_or(0.0);
            assert!(
                visited * newer["Actual Loops"].as_f64().unwrap() <= history as f64,
                "supersession must not scan unrelated methods: {plan}"
            );
            eprintln!(
                "metadata dense history: locked={}, transaction={transaction}, history={history}, visited={visited}, execution_ms={}",
                statement == LOCKED_CANDIDATE_SQL,
...
fn assert_supersession_index(root: &Value, rendered: &str) {
    let newer = find_alias(root, "newer").expect("eligibility rejects superseded approvals");
    assert_eq!(
        newer["Index Name"].as_str(),
        Some("billing_payment_attempts_payment_method_idx"),
        "supersession must use the method index:\n{rendered}"
    );
    assert!(
        newer["Index Cond"]
            .as_str()
            .is_some_and(|condition| condition.contains("payment_method_id = m.id")),
        "supersession must constrain method identity:\n{rendered}"
    );
}
