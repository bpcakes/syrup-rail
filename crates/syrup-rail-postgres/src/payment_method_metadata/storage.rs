...
pub(super) async fn lock_candidate(
    ...
    // The v5 cutover unifies scrub and approval on this subscriber's method
    // domain across plans. Enter it before the plan aggregate and row locks;
    // it stabilizes current references without holding locks during provider I/O.
    // Lifecycle-only attempt updates do not invalidate display, while method
    // and subscription timestamps fence intervening projection changes.
    crate::advisory_locks::lock_payment_method_domain(
        connection,
        command.subscriber_id,
        syrup_rail::GatewayAccountId::new(candidate.account_id),
    )
    .await?;
    let plan_key =
